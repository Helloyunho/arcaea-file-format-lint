...
class ToASTVisitor
  extends BaseAffVisitor
  implements ICstVisitor<AFFError[], any>
{
  constructor() {
    super()
  ...
  metadata(ctx: CstChildrenDictionary, errors: AFFError[]): AFFMetadata {
    ...
    const entries = ctx.metadataEntry
      ? (ctx.metadataEntry as CstNode[]).map((node) => ({
          node,
          entry: this.visit(node, errors) as AFFMetadataEntry
        }))
      : []
    ...
    return {
      data: metadata,
      metaEndLocation: locationFromToken(ctx.metaEnd[0] as IToken)
    }
  }
  values(
    ctx: CstChildrenDictionary,
    errors: AFFError[]
  ): WithLocation<AFFValue>[] {
    return ctx.value
      ? (ctx.value as IToken[]).map((token) => ({
          data: ((token): AFFValue => {
            if (token.tokenType === tokenTypes.word) {
              ...
              return {
                ...
                digit: token.image.length - token.image.indexOf('.') - 1
              }
            } else {
  ...
  event(ctx: CstChildrenDictionary, errors: AFFError[]): AFFEvent | null {
    // type checks
    const values = this.visit(
      ctx.values[0] as CstNode
    ) as WithLocation<AFFValue>[]
    const valuesLocation = (ctx.values[0] as CstNode).location
    const subevents = ctx.subevents
      ? (this.visit(
          ctx.subevents[0] as CstNode,
          errors
        ) as WithLocation<AFFEvent>[])
      : null
    const subeventsLocation = ctx.subevents
      ? (ctx.subevents[0] as CstNode).location
      : null
    const segment = ctx.segment
      ? (this.visit(
          ctx.segment[0] as CstNode,
          errors
        ) as WithLocation<AFFEvent>[])
      : null
    const segmentLocation = ctx.segment
      ? (ctx.segment[0] as CstNode).location
      : null
    if (ctx.word) {
      const tag = ctx.word[0] as IToken
      const tagLocation = locationFromToken(tag)
      if (tag.image === 'timing') {
        return eventTransformer.timing(
          errors,
          values,
          valuesLocation,
          subevents,
        ...
        return eventTransformer.arctap(
          errors,
          values,
          valuesLocation,
          subevents,
        ...
        return eventTransformer.camera(
          errors,
          values,
          valuesLocation,
          subevents,
        ...
        return eventTransformer.scenecontrol(
          errors,
          values,
          valuesLocation,
          subevents,
        ...
        return eventTransformer.timinggroup(
          errors,
          values,
          valuesLocation,
          subevents,
  ...
  subevents(
    ctx: CstChildrenDictionary,
    errors: AFFError[]
  ): WithLocation<AFFEvent>[] {
    return ctx.event
      ? (ctx.event as CstNode[])
          .map((node) => ({
  ...
  items(
    ...
    return ctx.item
      ? (ctx.item as CstNode[])
          .map(
            (node) => this.visit(node, errors) as WithLocation<AFFItem> | null
          )
          .filter((e) => e !== null)
      : []
  }
  aff(ctx: CstChildrenDictionary, errors: AFFError[]): AFFFile {
    ...
    const items = this.visit(
      ctx.items[0] as CstNode,
      errors
    ) as WithLocation<AFFItem>[]
    return {
      metadata: { data: metadata, location: metadataNode.location },
      items
    }
  }
}
...
const locationFromToken = (token: IToken): CstNodeLocation => {
  const { startColumn, startLine, startOffset, endColumn, endLine, endOffset } =
    token
...
const eventTransformer = {
  tap: (
    ...
    const rawTrackId = checkValueType(
      errors,
      'tap',
      'track-id',
      'int',
  ...
  arc: (
    ...
    const rawArcKind = checkValueType(
      errors,
      'arc',
      'arc-kind',
      'word',
    ...
    const rawColorId = checkValueType(
      errors,
      'arc',
      'color-id',
      'int',
    ...
    const rawIsLine = checkValueType(
      errors,
      'arc',
      'is-line',
      'word',
    ...
    return {
      ...
      isLine,
      arctaps: subevents
        ? transformArcSubevents(errors, subevents, subeventsLocation)
        : undefined,
      tagLocation
    }
  },
  camera: (
    ...
    const translationX = checkValueType(
      errors,
      'camera',
      'translation-x',
      'float',
    ...
    const translationY = checkValueType(
      errors,
      'camera',
      'translation-y',
      'float',
    ...
    const translationZ = checkValueType(
      errors,
      'camera',
      'translation-z',
      'float',
    ...
    const rotationX = checkValueType(
      errors,
      'camera',
      'rotation-x',
      'float',
    ...
    const rotationY = checkValueType(
      errors,
      'camera',
      'rotation-y',
      'float',
    ...
    const rotationZ = checkValueType(
      errors,
      'camera',
      'rotation-z',
      'float',
    ...
    const rawCameraKind = checkValueType(
      errors,
      'camera',
      'camera-kind',
      'word',
    ...
    const cameraKind = parseValue.cameraKind(
      errors,
      'camera',
      'camera-kind',
      rawCameraKind
  ...
  scenecontrol: (
    ...
    const rawSceneControlKind = checkValueType(
      errors,
      'scenecontrol',
      'scenecontrol-kind',
      'word',
    ...
    const sceneControlKind = parseValue.sceneControlKind(
      errors,
      'scenecontrol',
      'scenecontrol-kind',
      rawSceneControlKind
  ...
  timinggroup: (
    ...
    const rawTimingGroupKind =
      values.length > 0
        ...
        : null
    const maybeTimingGroupKind = parseValue.timingGroupKind(
      errors,
      'timinggroup',
      'timinggroup-kind',
      rawTimingGroupKind
    ...
    return {
      kind: 'timinggroup',
      items: selectNestedItems(errors, segment, segmentLocation),
      tagLocation,
      timingGroupAttribute: timingGroupKind
    }
  }
}
...
const parseValue = {
  trackId: (
    ...
    eventKind: string,
    fieldName: string,
    int: WithLocation<AFFInt> | null
  ): WithLocation<AFFTrackId> => {
    if (int) {
      ...
      return {
        data: { kind: 'track-id', value: intValue } as AFFTrackId,
        location
      }
    } else {
      return null
  ...
  colorId: (
    ...
    if (int) {
      ...
      return {
        data: { kind: 'color-id', value: intValue } as AFFColorId,
        location
      }
    } else {
      return null
  ...
  arcKind: (
    ...
    eventKind: string,
    fieldName: string,
    word: WithLocation<AFFWord> | null
  ): WithLocation<AFFArcKind> => {
    if (word) {
      ...
      return {
        data: { kind: 'arc-kind', value: wordValue } as AFFArcKind,
        location
      }
    } else {
      return null
  ...
  effect: (
    ...
    _fieldName: string,
    word: WithLocation<AFFWord> | null
  ): WithLocation<AFFEffect> => {
    if (word) {
      const { data, location } = word
      const wordValue = data.value
      return {
        data: { kind: 'effect', value: wordValue } as AFFEffect,
        location
      }
    } else {
      return null
  ...
  bool: (
    ...
    if (word) {
      ...
      return {
        data: { kind: 'bool', value: wordValue === 'true' } as AFFBool,
        location
      }
    } else {
      return null
  ...
  cameraKind: (
    ...
    if (word) {
      ...
      return {
        data: { kind: 'camera-kind', value: wordValue } as AFFCameraKind,
        location
      }
    } else {
      return null
  ...
  sceneControlKind: (
    ...
    fieldName: string,
    word: WithLocation<AFFWord> | null
  ): WithLocation<AFFSceneControlKind> => {
    if (word) {
      const { data, location } = word
      const wordValue = data.value
      return {
  ...
  timingGroupKind: (
    ...
    fieldName: string,
    word: WithLocation<AFFWord> | null
  ): WithLocation<AFFTimingGroupKind> => {
    if (word) {
      const { data, location } = word
      const wordValue = data.value
      return {
