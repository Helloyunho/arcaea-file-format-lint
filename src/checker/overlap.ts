...
export const overlapChecker: AFFChecker = (file, error) => {
        ...
        for (const arctap of arctaps.data) {
          ...
          if (timestamps.has(timestamp)) {
            error.push({
              ...
              type: AFFErrorType.DuplicatedArcTap,
  ...
  const scenecontrolKinds = new Map<
    string,
    WithLocation<AFFSceneControlEvent>[]
  >()
  const processScenecontrol = (
    scenecontrol: WithLocation<AFFSceneControlEvent>
  ) => {
    ...
    if (['enwidencamera', 'enwidenlanes'].includes(kind)) {
      ...
      if (values.data.length === 2) {
        if (
          values.data[0].data.kind == 'float' &&
          values.data[1].data.kind == 'int'
        ) {
...
const checkTrackOverlap = (
  error: AFFError[],
  items: WithLocation<AFFTrackItem>[]
) => {
  const getStart = (item: WithLocation<AFFTrackItem>) =>
    item.data.kind === 'tap'
      ? item.data.time.data.value
      : item.data.start.data.value
  const report = (location: CstNodeLocation, lastLocation: CstNodeLocation) => {
    error.push({
      ...
      type: AFFErrorType.OverlappedItem,
      severity: AFFErrorLevel.Error,
      location,
      relatedInfo: [
        {
...
const checkCameraOverlap = (
  error: AFFError[],
  cameras: WithLocation<AFFCameraEvent>[]
) => {
  const report = (location: CstNodeLocation, lastLocation: CstNodeLocation) => {
    error.push({
  ...
  const sortedByStart = cameras.sort(
    (a, b) => a.data.time.data.value - b.data.time.data.value
  )
  let lastLocation: CstNodeLocation | null = null
  let lastEnd: number = -Infinity
  for (const item of sortedByStart) {
...
const checkScenecontrolOverlap = (
  error: AFFError[],
  scenecontrols: WithLocation<AFFSceneControlEvent>[],
  kind: string
) => {
  const report = (location: CstNodeLocation, lastLocation: CstNodeLocation) => {
    error.push({
  ...
  const sortedByStart = scenecontrols.sort(
    (a, b) => a.data.time.data.value - b.data.time.data.value
  )
  let lastLocation: CstNodeLocation | null = null
  let lastEnd: number = -Infinity
  for (const item of sortedByStart) {
    ...
    const end =
      start + (item.data.values.data[0] as WithLocation<AFFFloat>).data.value
    if (end > lastEnd) {
      lastLocation = item.location
      lastEnd = end
    }
