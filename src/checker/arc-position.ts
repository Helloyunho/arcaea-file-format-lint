...
const checkItem = (
  ...
  if (data.kind === 'arc') {
    ...
    checkPoint(
      'start',
      solid,
      data.xStart.data.value,
      data.yStart.data.value,
    ...
    checkPoint(
      'end',
      solid,
      data.xEnd.data.value,
      data.yEnd.data.value,
      data.end.data.value,
      cameras,
      location,
      errors
...
const checkPoint = (
  ...
  let lastEnwidenCameraId =
    upperBound(cameras, time, (ec, t) => ec.time - t) - 1
  if (lastEnwidenCameraId >= 0) {
    if (
      !cameras[lastEnwidenCameraId].enabled &&
      cameras[lastEnwidenCameraId].time === time
    ) {
