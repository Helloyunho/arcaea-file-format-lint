  ...
  for (const item of file.items) {
    if (item.data.kind === 'tap') {
      ...
      if (trackId.data.value === 0 || trackId.data.value === 5) {
        let lastEnwidenlaneId =
          upperBound(lanes, item.data.time.data.value, (ed, t) => ed.time - t) -
          1
        if (lastEnwidenlaneId >= 0) {
          if (
            !lanes[lastEnwidenlaneId].enabled &&
            lanes[lastEnwidenlaneId].time === item.data.time.data.value
          ) {
            lastEnwidenlaneId -= 1
          }
        }
            ...
            relatedInfo: [
              {
                ...
                location:
                  lanes[lastEnwidenlaneId]?.item?.location ??
                  file.metadata.data.metaEndLocation
              }
            ]
          })
        }
      ...
      const trackId = item.data.trackId
      if (trackId.data.value === 0 || trackId.data.value === 5) {
        const firstEnwidenlaneId =
          upperBound(
            lanes,
            item.data.start.data.value,
            (ed, t) => ed.time - t
          ) - 1
        const lastEnwidenlaneId = lowerBound(
          lanes,
          item.data.end.data.value,
          (ed, t) => ed.time - t
        )
        const disabler: (EnwidenData | null)[] = lanes
          .slice(Math.max(firstEnwidenlaneId, 0), lastEnwidenlaneId)
        ...
        if (disabler.length > 0) {
          errors.push({
            ...
            relatedInfo: disabler.map((lane) => ({
              message: `The scenecontrol event that disable enwidenlanes`,
              location:
                lane?.item?.location ?? file.metadata.data.metaEndLocation
            }))
          })
        }
      }
