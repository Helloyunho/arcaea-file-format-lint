    ...
    if (data.kind === 'scenecontrol') {
      ...
      for (const nestedItem of data.items.data) {
        if (nestedItem.data.kind === 'scenecontrol') {
          checkScenecontrol(
            nestedItem.data.sceneControlKind,
            nestedItem.data.values,
            errors
          )
        }
      }
...
const checkScenecontrol = (
  kind: WithLocation<AFFSceneControlKind>,
  ...
  if (
    ...
    if (
      checkValuesCount(error, kind.data.value, 2, values.data, values.location)
    ) {
