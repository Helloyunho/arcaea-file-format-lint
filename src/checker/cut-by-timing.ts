...
import { AFFChecker, AFFError, AFFErrorLevel, AFFErrorType } from '../types.js'
      ...
      for (const nestedItem of item.data.items.data) {
        if (nestedItem.data.kind === 'hold') {
          checkCutByTiming(
            'hold',
            groupTimingData,
            nestedItem.data.start.data.value,
            nestedItem.data.end.data.value,
            nestedItem.location,
            error
          )
...
const checkCutByTiming = (
  ...
  if (cuters.length > 0) {
    error.push({
      ...
      type: AFFErrorType.ItemCutByTiming,
