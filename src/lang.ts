...
export const checkAFF = (content: TextDocument): lsp.Diagnostic[] => {
    ...
    errors = errors.concat(
      lexingResult.errors.map((e) => ({
        ...
        range: {
          ...
          end: content.positionAt(
            content.offsetAt({ line: e.line - 1, character: e.column - 1 }) +
              e.length
          )
        }
      }))
  ...
  if (affParser.errors.length > 0) {
    ...
    errors = errors.concat(
      astResult.errors.map((e) => transformAFFError(e, content.uri))
    )
    const checkerErrors = processCheckers(astResult.ast)
    errors = errors.concat(
      checkerErrors.map((e) => transformAFFError(e, content.uri))
    )
  }
  return errors
}
